...
export function PrizesScreen({
  ...
  const [sheetPrize, setSheetPrize] = useState<MockPrize | null>(null);

  function handleOpenPrize(prize: MockPrize) {
    ...
    <>
      ...
      {sheetPrize ? (
        <BottomSheet
          ...
          onClose={() => setSheetPrize(null)}
          title={sheetPrize.title}
        >
          <div className="flex items-center justify-between gap-3">
            <p className="text-sm leading-6 text-[var(--muted-strong)]">
              {sheetPrize.description}
            </p>
            <Badge className="shrink-0 px-2.5 text-[11px]">
              {prizeStatusLabel[sheetPrize.status]}
            </Badge>
          </div>
          ...
          <div className="mt-4 rounded-2xl bg-white/[0.05] px-3 py-2.5">
            ...
            <p className="mt-2 text-xs leading-5 text-[var(--muted)]">
              Реальная выдача будет подключена позже.
            </p>
          </div>
